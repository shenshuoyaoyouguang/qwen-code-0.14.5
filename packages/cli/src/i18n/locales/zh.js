...
export default {
  ...
  // ============================================================================
  'Manage MCP servers': '管理 MCP 服务器',
  'Manage DM pairing requests': '管理 DM 配对请求',
  'Manage messaging channels (Telegram, Discord, etc.)':
    '管理消息通道（Telegram、Discord 等）',
  'Manage Qwen Code hooks (use /hooks in interactive mode).':
    '管理 Qwen Code 钩子（交互模式下请使用 /hooks）。',
  'Manage Qwen Code extensions.': '管理 Qwen Code 扩展。',
  'You need at least one command before continuing.':
    '至少需要一个子命令才能继续。',
  'Add a server': '添加服务器',
  'Remove a server': '移除服务器',
  'List all configured MCP servers': '列出所有已配置的 MCP 服务器',
  'Reconnect MCP server(s)': '重新连接 MCP 服务器',
  'Name of the server': '服务器名称',
  'Command (stdio) or URL (sse, http)': '命令（stdio）或 URL（sse、http）',
  'Configuration scope (user or project)': '配置范围（user 或 project）',
  'Transport type (stdio, sse, http). Auto-detected from URL if not specified.':
    '传输类型（stdio、sse、http）。如未指定，将根据 URL 自动检测。',
  ...
  'Set HTTP headers for SSE and HTTP transports (e.g. -H "X-Api-Key: abc123" -H "Authorization: Bearer abc123")':
    '设置 SSE 和 HTTP 传输的 HTTP 请求头（例如 -H "X-Api-Key: abc123" -H "Authorization: Bearer abc123"）',
  'Set connection timeout in milliseconds': '设置连接超时时间（毫秒）',
  'Trust the server (bypass all tool call confirmation prompts)':
    '信任该服务器（跳过所有工具调用确认提示）',
  'Set the description for the server': '设置服务器描述',
  'A comma-separated list of tools to include': '要包含的工具列表（逗号分隔）',
  'A comma-separated list of tools to exclude': '要排除的工具列表（逗号分隔）',
  'Name of the server to reconnect': '要重新连接的服务器名称',
  'Reconnect all configured servers': '重新连接所有已配置的服务器',
  'Stop the running channel service': '停止运行中的通道服务',
  'Show channel service status': '显示通道服务状态',
  'Start channels (all if no name given, or a single named channel)':
    '启动通道（未指定名称时启动全部，或启动指定通道）',
  'Channel name (omit to start all configured channels)':
    '通道名称（省略则启动所有已配置的通道）',
  'List pending pairing requests for a channel':
    '列出通道的待处理配对请求',
  'Channel name': '通道名称',
  'Approve a pending pairing request': '批准待处理的配对请求',
  'Pairing code': '配对码',
  'Configure WeChat channel (login via QR code)':
    '配置微信通道（通过二维码登录）',
  '"clear" to remove stored credentials, omit to login':
    '"clear" 清除已存储的凭据，省略则登录',
  'Create a new extension from a boilerplate example.':
    '从模板示例创建新扩展。',
  'The path to create the extension in.': '创建扩展的路径。',
  'The boilerplate template to use.': '要使用的模板。',

  // ============================================================================
  // Yargs CLI 选项 (config.ts)
  // ============================================================================
  'Enable telemetry? This flag specifically controls if telemetry is sent. Other --telemetry-* flags set specific values but do not enable telemetry on their own.':
    '启用遥测？此标志专门控制是否发送遥测数据。其他 --telemetry-* 标志设置特定值，但不会自行启用遥测。',
  ...
  'Redirect all telemetry output to the specified file.':
    '将所有遥测输出重定向到指定文件。',
  'Run in debug mode?': '以调试模式运行？',
  'Proxy for Qwen Code, like schema://user:password@host:port':
    'Qwen Code 代理地址，如 schema://user:password@host:port',
  'Enable chat recording to disk. If false, chat history is not saved and --continue/--resume will not work.':
    '启用聊天记录保存到磁盘。如果为 false，聊天历史不会保存，--continue/--resume 将无法使用。',
  'Launch Qwen Code CLI': '启动 Qwen Code CLI',
  'Positional prompt. Defaults to one-shot; use -i/--prompt-interactive for interactive.':
    '位置参数提示。默认为单次执行；使用 -i/--prompt-interactive 进入交互模式。',
  Model: '模型',
  ...
  'Override the main session system prompt for this run. Can be combined with --append-system-prompt.':
    '覆盖本次运行的主会话系统提示。可与 --append-system-prompt 结合使用。',
  'Append instructions to the main session system prompt for this run. Can be combined with --system-prompt.':
    '向本次运行的主会话系统提示追加指令。可与 --system-prompt 结合使用。',
  'Run in sandbox?': '在沙箱中运行？',
  'Sandbox image URI.': '沙箱镜像 URI。',
  'Automatically accept all actions (aka YOLO mode, see https://www.youtube.com/watch?v=xvFZjo5PgG0 for more details)?':
    '自动接受所有操作（即 YOLO 模式，详见 https://www.youtube.com/watch?v=xvFZjo5PgG0）？',
  'Set the approval mode: plan (plan only), default (prompt for approval), auto-edit (auto-approve edit tools), yolo (auto-approve all tools)':
    '设置审批模式：plan（仅规划）、default（需要审批）、auto-edit（自动批准编辑工具）、yolo（自动批准所有工具）',
  'Enables checkpointing of file edits': '启用文件编辑检查点',
  'Starts the agent in ACP mode': '以 ACP 模式启动智能体',
  'Starts the agent in ACP mode (deprecated, use --acp instead)':
    '以 ACP 模式启动智能体（已弃用，请使用 --acp）',
  'Deprecated: Skills are now enabled by default. This flag is ignored.':
    '已弃用：技能现在默认启用。此标志将被忽略。',
  'Enable experimental LSP (Language Server Protocol) feature for code intelligence':
    '启用实验性 LSP（语言服务器协议）功能以提供代码智能',
  'Channel identifier (VSCode, ACP, SDK, CI)':
    '通道标识符（VSCode、ACP、SDK、CI）',
  ...
  'Additional directories to include in the workspace (comma-separated or multiple --include-directories)':
    '要包含在工作空间中的额外目录（逗号分隔或多次使用 --include-directories）',
  ...
  'Custom directory path for OpenAI API logs. Overrides settings files.':
    'OpenAI API 日志的自定义目录路径。覆盖设置文件。',
  'OpenAI API key to use for authentication': '用于认证的 OpenAI API 密钥',
  'OpenAI base URL (for custom endpoints)': 'OpenAI 基础 URL（用于自定义端点）',
  'Tavily API key for web search': 'Tavily 网页搜索 API 密钥',
  'Google Custom Search API key': 'Google 自定义搜索 API 密钥',
  'Google Custom Search Engine ID': 'Google 自定义搜索引擎 ID',
  'Default web search provider (dashscope, tavily, google)':
    '默认网页搜索提供商（dashscope、tavily、google）',
  'Enable screen reader mode for accessibility.':
    '启用屏幕阅读器模式以提升无障碍体验。',
  'The format consumed from standard input.':
    '从标准输入消费的格式。',
  'The format of the CLI output.': 'CLI 输出的格式。',
  'Include partial assistant messages when using stream-json output.':
    '使用 stream-json 输出时包含部分助手消息。',
  'Resume the most recent session for the current project.':
    '恢复当前项目的最近一次会话。',
  'Resume a specific session by its ID. Use without an ID to show session picker.':
    '通过 ID 恢复特定会话。不带 ID 则显示会话选择器。',
  'Specify a session ID for this run.': '指定本次运行的会话 ID。',
  'Maximum number of session turns': '会话最大轮数',
  'Core tool paths': '核心工具路径',
  'Tools to exclude': '要排除的工具',
  ...
  // ============================================================================
  'tool_desc:read_file':
    '读取并返回指定文件的内容。支持文本、图片和 PDF 文件。',
  'tool_desc:write_file':
    '将内容写入本地文件系统中的指定文件。',
  'tool_desc:edit':
    '替换文件中的文本。将 old_string 的单个匹配项替换为 new_string。',
  'tool_desc:run_shell_command':
    '在持久化 shell 会话中执行给定的 shell 命令，支持可选超时。',
  'tool_desc:grep_search':
    '在文件或目录的内容中搜索正则表达式模式。',
  'tool_desc:ripgrep_search':
    '使用 ripgrep 在文件内容中搜索正则表达式模式。',
  'tool_desc:glob':
    '高效查找匹配特定 glob 模式的文件。',
  'tool_desc:list_directory':
    '列出指定目录路径下的文件和子目录名称。',
  'tool_desc:save_memory':
    '将特定信息或事实保存到长期记忆中。',
  'tool_desc:todo_write':
    '创建或更新当前编码会话的结构化任务列表。',
  'tool_desc:web_fetch':
    '读取网页完整内容并转换为简洁的 Markdown 格式。',
  'tool_desc:web_search':
    '搜索任意主题并获取简洁、可直接使用的内容。',
  'tool_desc:agent':
    '启动新的智能体来自主处理复杂的多步骤任务。',
  'tool_desc:skill':
    '在主对话中执行技能。',
  'tool_desc:exit_plan_mode':
    '当您处于规划模式并完成计划展示后使用此工具。',
  'tool_desc:ask_user_question':
    '在执行过程中需要向用户提问时使用此工具。',
  'tool_desc:lsp':
    '语言服务器协议工具，提供代码智能功能。',
  'tool_desc:cron_create':
    '创建新的定时任务以进行计划执行。',
  'tool_desc:cron_list':
    '列出所有已配置的定时任务。',
  'tool_desc:cron_delete':
    '删除现有的定时任务。',
};
