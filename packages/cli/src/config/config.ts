...
export async function parseArguments(): Promise<CliArgs> {
  ...
  const yargsInstance = yargs(rawArgv)
    ...
    .option('telemetry-outfile', {
      type: 'string',
      description: t('Redirect all telemetry output to the specified file.'),
    })
    .deprecateOption(
      'telemetry',
    ...
    .option('debug', {
      alias: 'd',
      type: 'boolean',
      description: t('Run in debug mode?'),
      default: false,
    })
    .option('proxy', {
      type: 'string',
    ...
    .command('$0 [query..]', t('Launch Qwen Code CLI'), (yargsInstance: Argv) =>
      yargsInstance
        .positional('query', {
          description: t(
        ...
        .option('prompt', {
          alias: 'p',
          type: 'string',
          description: t('Prompt. Appended to input on stdin (if any).'),
        })
        ...
        .option('sandbox', {
          alias: 's',
          type: 'boolean',
          description: t('Run in sandbox?'),
        })
        .option('sandbox-image', {
          type: 'string',
          description: t('Sandbox image URI.'),
        })
        .option('yolo', {
          alias: 'y',
          type: 'boolean',
        ...
        .option('checkpointing', {
          type: 'boolean',
          description: t('Enables checkpointing of file edits'),
          default: false,
        })
        .option('acp', {
          type: 'boolean',
          description: t('Starts the agent in ACP mode'),
        })
        ...
        .option('channel', {
          ...
          description: t('Channel identifier (VSCode, ACP, SDK, CI)'),
        })
        .option('allowed-mcp-server-names', {
          type: 'array',
          string: true,
          description: t('Allowed MCP server names'),
        ...
        .option('allowed-tools', {
          type: 'array',
          string: true,
          description: t('Tools that are allowed to run without confirmation'),
          coerce: (tools: string[]) =>
            // Handle comma-separated values
        ...
        .option('list-extensions', {
          alias: 'l',
          type: 'boolean',
          description: t('List all available extensions and exit.'),
        })
        ...
        .option('openai-api-key', {
          type: 'string',
          description: t('OpenAI API key to use for authentication'),
        })
        .option('openai-base-url', {
          type: 'string',
          description: t('OpenAI base URL (for custom endpoints)'),
        })
        .option('tavily-api-key', {
          type: 'string',
          description: t('Tavily API key for web search'),
        })
        .option('google-api-key', {
          type: 'string',
          description: t('Google Custom Search API key'),
        })
        .option('google-search-engine-id', {
          type: 'string',
          description: t('Google Custom Search Engine ID'),
        })
        ...
        .option('screen-reader', {
          type: 'boolean',
          description: t('Enable screen reader mode for accessibility.'),
        })
        .option('input-format', {
          type: 'string',
          choices: ['text', 'stream-json'],
          description: t('The format consumed from standard input.'),
          default: 'text',
        })
        .option('output-format', {
          alias: 'o',
          type: 'string',
          description: t('The format of the CLI output.'),
        ...
        .option('session-id', {
          type: 'string',
          description: t('Specify a session ID for this run.'),
        })
        .option('max-session-turns', {
          type: 'number',
          description: t('Maximum number of session turns'),
        })
        .option('core-tools', {
          type: 'array',
          string: true,
          description: t('Core tool paths'),
          coerce: (tools: string[]) =>
            tools.flatMap((tool) => tool.split(',').map((t) => t.trim())),
        })
        .option('exclude-tools', {
          type: 'array',
          string: true,
          description: t('Tools to exclude'),
          coerce: (tools: string[]) =>
            tools.flatMap((tool) => tool.split(',').map((t) => t.trim())),
        })
        .option('allowed-tools', {
          type: 'array',
          string: true,
          description: t('Tools to allow, will bypass confirmation'),
          coerce: (tools: string[]) =>
            tools.flatMap((tool) => tool.split(',').map((t) => t.trim())),
        })
        .option('auth-type', {
          ...
          description: t('Authentication type'),
        })
        .deprecateOption(
          'sandbox-image',
...
export async function loadCliConfig(
  ...
  const config = new Config({
    ...
    agents: settings.agents
      ...
      : undefined,
    trellis: settings.context?.trellis,
  });
