  ...
  context: {
    ...
    properties: {
      ...
      trellis: {
        ...
        requiresRestart: false,
        default: {},
        description: 'Trellis 任务管理系统配置。',
        showInDialog: false,
        properties: {
          enabled: {
            type: 'boolean',
            label: 'Enable Trellis',
            category: 'Context',
            requiresRestart: false,
            default: false,
            description:
              '启用 Trellis 任务管理功能，包括会话记录和任务上下文注入。',
            showInDialog: false,
          },
          autoCreateJournal: {
            type: 'boolean',
            ...
            requiresRestart: false,
            default: true,
            description: '当启动任务时自动创建 journal 文件记录会话。',
            showInDialog: false,
          },
          maxJournalLines: {
            type: 'number',
            label: 'Max Journal Lines',
            category: 'Context',
            requiresRestart: false,
            default: 2000,
            description:
              '每个 journal 文件的最大行数。超过此值时自动创建新文件。',
            showInDialog: false,
          },
          autoCommitOnFinish: {
            type: 'boolean',
            label: 'Auto Commit On Finish',
            category: 'Context',
            requiresRestart: false,
            default: true,
            description: '完成任务时自动提交 journal 和 workspace 变更到 git。',
            showInDialog: false,
          },
          injectContextOnStart: {
            type: 'boolean',
            label: 'Inject Context On Start',
            category: 'Context',
            requiresRestart: false,
            default: true,
            description: '会话启动时自动注入当前任务上下文。',
            showInDialog: false,
          },
        },
      },
